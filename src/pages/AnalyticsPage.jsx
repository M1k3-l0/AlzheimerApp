        ...
        if (active && payload && payload.length) {
            ...
            return (
                <div style={{ backgroundColor: 'white', padding: '10px', border: '1px solid #eee', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>
                    <p style={{ margin: 0, fontWeight: 'bold', fontSize: '0.75rem' }}>{payload[0].payload.time}</p>
    ...
    return (
        <div style={styles.container}>
                ...
                <div style={styles.chartContainer}>
                    {isReady && moodData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={250}>
                            ...
                            <XAxis dataKey="time" axisLine={false} tickLine={false} tick={{ fontSize: '0.625rem', fill: '#9CA3AF' }} />
                            <YAxis domain={[1, 3]} ticks={[1, 2, 3]} axisLine={false} tickLine={false} tick={{ fontSize: '0.625rem', fill: '#9CA3AF' }} />
                            <Tooltip content={<CustomTooltip />} />
                            <Area type="monotone" dataKey="val" stroke="var(--color-primary)" strokeWidth={3} fillOpacity={1} fill="url(#colorMood)" />
                        </AreaChart>
                    </ResponsiveContainer>
                    ) : (
                        <div style={{ height: 250, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9CA3AF', fontSize: '0.75rem' }}>
                ...
                <div style={styles.chartContainer}>
                    {isReady && taskData.length > 0 ? (
                        <ResponsiveContainer width="100%" height={250}>
                            ...
                            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F3F4F6" />
                            <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fontSize: '0.625rem', fill: '#9CA3AF' }} />
                            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: '0.625rem', fill: '#9CA3AF' }} />
            ...
            <div style={{ ...styles.card, background: 'linear-gradient(135deg, #EEF2FF 0%, #E0E7FF 100%)', border: '1px solid #C7D2FE' }}>
                ...
                <p style={{ fontSize: '0.8125rem', color: '#4338CA', lineHeight: '1.6', margin: 0 }}>
