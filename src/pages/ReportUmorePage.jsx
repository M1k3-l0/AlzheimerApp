...
export default function ReportUmorePage() {
  ...
  const styles = {
    ...
    empty: { textAlign: 'center', color: 'var(--color-text-secondary)', padding: '32px 16px', fontSize: '0.9375rem' },
          ...
          {lastEntry ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontWeight: '600', fontSize: '1.125rem' }}>{MOOD_LABELS[lastEntry.mood]}</span>
              </span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--color-text-secondary)', fontSize: '0.9375rem' }}>
        ...
        <div style={styles.card}>
          ...
          {monthlyData.some((d) => d.mood) ? (
            <>
              ...
              <div style={{ display: 'flex', gap: '16px', marginTop: '16px', fontSize: '0.8125rem' }}>
                ...
                {isReady && (
                  <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={50}>
                    <BarChart data={monthlyData} margin={{ top: 8, right: 8, left: 8, bottom: 8 }}>
                      ...
                      <XAxis dataKey="day" tick={{ fontSize: '0.6875rem' }} />
                      <YAxis tick={{ fontSize: '0.6875rem' }} allowDecimals={false} />
              ...
              {isReady && (
                <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0} debounce={50}>
                  <LineChart data={annualData} margin={{ top: 8, right: 8, left: 8, bottom: 8 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                    <XAxis dataKey="month" tick={{ fontSize: '0.6875rem' }} />
                    <YAxis domain={[0, 100]} tick={{ fontSize: '0.6875rem' }} />
